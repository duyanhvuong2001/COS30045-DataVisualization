function init() {
  ...
  for (var i = 0; i < stateButtons.length; i++) {
    stateButtons[i].onclick = function () {
      ...
      var filepath =
        "data/" + this.textContent + "ElectricityGeneration08-21.csv"; //concatenate strings based on buttons clicked
      this.addEventListener(
        "click",
        changeData(filepath, w, h, svg, ANIMATION_DURATION)
      );
    };
  }
}
...
function prepareGraph(filepath, w, h, svg) {
  ...
  d3.csv(filepath, rowConverter, function (error, data) {
    ...
    var xScale = d3
      .scaleTime() //x axis and scale is the year
      .domain([
    ...
    var yScale = d3
      .scaleLinear() //y axis and scale is the data
      .domain([
        0,
    ...
    //draw xAxis
    var xAxis = d3.axisBottom().scale(xScale).ticks(dataset.length);
...
function changeData(filepath, w, h, svg, animation_duration) {
  ...
  d3.csv(filepath, rowConverter, function (error, data) {
    ...
    var series = svg.selectAll("g.series > path").data(stackedData);
    series
      .attr("fill", function (d, i) {
        return color[d.key];
      })
